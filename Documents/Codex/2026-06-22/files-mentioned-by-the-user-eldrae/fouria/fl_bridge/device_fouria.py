...
"""FOURIA native FL Studio control bridge."""
import json
import time
...
def _post(path, payload, timeout=4):
    req = Request(FOURIA_URL + path, data=json.dumps(payload).encode("utf-8"),
...
def _get(path, timeout=3):
    with urlopen(FOURIA_URL + path, timeout=timeout) as response:
        return json.loads(response.read().decode("utf-8"))
...
def _plugin_names(channel_index):
    return [_safe(lambda: plugins.getPluginName(channel_index), "")]
...
def _role_level(name):
    ...
    if any(word in text for word in ("808", "bass", "sub")): return 0.72
    if any(word in text for word in ("snare", "clap")): return 0.70
    if any(word in text for word in ("hat", "shaker", "perc")): return 0.58
    if any(word in text for word in ("vocal", "vox", "lead")): return 0.68
    if any(word in text for word in ("melody", "keys", "piano", "guitar", "synth", "pad")): return 0.62
    return 0.64
...
def _gain_stage():
    ...
    return {"gain_staged": changed, "note": "Initial static balance only; audible mix decisions still require playback/reference checks."}
...
def _execute(item):
    ...
    if not _safe(lambda: general.safeToEdit(), 1):
        raise RuntimeError("FL Studio is not currently safe to edit")
    _undo(action.replace("_", " "))
    index = int(value.get("index", -1))
    if action == "organize_project": return _organize_project()
    if action == "gain_stage_mix": return _gain_stage()
    if action == "set_channel_name": channels.setChannelName(index, str(value["name"])[:64], True)
    elif action == "set_channel_volume": channels.setChannelVolume(index, _clamp(float(value["volume"]), 0, 1), 0, True)
    elif action == "set_channel_pan": channels.setChannelPan(index, _clamp(float(value["pan"]), -1, 1), 0, True)
    elif action == "set_channel_pitch": channels.setChannelPitch(index, float(value["semitones"]), 1, 0, True)
    elif action == "mute_channel": channels.muteChannel(index, int(bool(value.get("enabled", True))), True)
    elif action == "solo_channel": channels.soloChannel(index, int(bool(value.get("enabled", True))), True)
    elif action == "select_channel": channels.selectOneChannel(index, True)
    elif action == "route_channel": channels.setTargetFxTrack(index, int(value["mixer_track"]), True)
    elif action == "quantize_channel": channels.quickQuantize(index, int(bool(value.get("start_only", True))), True)
    ...
    elif action == "set_mixer_name": mixer.setTrackName(index, str(value["name"])[:64])
    elif action == "set_mixer_volume": mixer.setTrackVolume(index, _clamp(float(value["volume"]), 0, 1))
    elif action == "set_mixer_pan": mixer.setTrackPan(index, _clamp(float(value["pan"]), -1, 1))
    elif action == "set_mixer_stereo": mixer.setTrackStereoSep(index, _clamp(float(value["separation"]), -1, 1))
    elif action == "mute_mixer": mixer.muteTrack(index, int(bool(value.get("enabled", True))))
    elif action == "solo_mixer": mixer.soloTrack(index, int(bool(value.get("enabled", True))))
    elif action == "select_mixer": mixer.setActiveTrack(index)
    elif action == "route_mixer": mixer.setRouteTo(index, int(value["destination"]), int(bool(value.get("enabled", True))), True)
    elif action == "set_route_level": mixer.setRouteToLevel(index, int(value["destination"]), _clamp(float(value["level"]), 0, 1))
    elif action == "set_plugin_mix": mixer.setPluginMixLevel(index, int(value["slot"]), _clamp(float(value["mix"]), 0, 1))
    elif action == "set_plugin_param":
        plugins.setParamValue(_clamp(float(value["value"]), 0, 1), int(value["param"]), index, int(value.get("slot", -1)), 0, True)
    elif action == "next_preset": plugins.nextPreset(index, int(value.get("slot", -1)), True)
    elif action == "previous_preset": plugins.prevPreset(index, int(value.get("slot", -1)), True)
    elif action == "set_pattern_name": patterns.setPatternName(index, str(value["name"])[:64])
    elif action == "select_pattern": patterns.jumpToPattern(index)
    elif action == "set_playlist_name": playlist.setTrackName(index, str(value["name"])[:64])
    elif action == "mute_playlist": playlist.muteTrack(index, int(bool(value.get("enabled", True))))
    elif action == "solo_playlist": playlist.soloTrack(index, int(bool(value.get("enabled", True))))
    else:
...
def OnInit():
    print("FOURIA bridge v" + BRIDGE_VERSION + " loaded.")
    _safe(lambda: ui.setHintMsg("FOURIA native control bridge connected."))


def OnIdle():
    global _last_sync, _last_action
    now = time.time()
    if now - _last_sync < 2:
        return
    _last_sync = now
    try:
        _post("/api/fl/sync", {"bridge_version": BRIDGE_VERSION, "project": _snapshot()}, 8)
        data = _get("/api/fl/actions?since=" + str(_last_action))
        for item in data.get("actions", []):
            ...
            _last_action = max(_last_action, int(item.get("id", 0)))
    except Exception:
        pass
